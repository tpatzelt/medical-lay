...
class UMLS_CUIs:
    ...
    def __init__(self, language: str = "GER"):
        """Initializes the class to create the UMLS dictionary of preferred terms
        """
        self.structure = {}
        self.language = language
        self.cui = {'SNOMEDCT_US': 1, 'MSH': 2, 'MDR': 3, 'RXNORM': 4}
    ...
    def __addCUI(self, umls_cui, language, kb, term_type, kb_cui, kb_name):
        ...
        if language == self.language and ((kb in ('RXNORM', 'MSH')) or term_type in (
                'PT', 'MH', 'SCD', 'MIN')) and kb in self.cui:
